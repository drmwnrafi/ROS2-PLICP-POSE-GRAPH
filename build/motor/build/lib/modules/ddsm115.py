...
class MotorControl:
	...
	def __init__(self, device="/dev/ttyUSB0"):
		...
		self.ser = serial.rs485.RS485(device, 115200, timeout=0, rtscts=False, dsrdtr=False)
	...
	def positionRawToPositionDeg(self, pos_raw: int):
		return self.map(pos_raw, 0, 255, 0, 360)
	...
	def read_reply(self, _id, timeout=0.01):
				...
				## size of ring_buffer not 10, and still some place to fill
					...
					if (len(ring_buffer) == 10):
						...
						if crc_value == crc_check:
							...
							fb_pos = self.positionRawToPositionDeg(pos)
		...
		return fb_rpm, fb_cur, int(fb_pos), error